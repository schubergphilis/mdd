...
def resolve_secret(value: str, *, account: str | None = None) -> str:
    ...
    try:
        ...
        raise SecretError(
            "op CLI not installed; install the 1Password CLI and sign in, then retry"
        ) from exc

    if result.returncode == 127:
        raise SecretError("op CLI not installed; install the 1Password CLI and sign in, then retry")
