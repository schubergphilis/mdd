...
def _cluster_summaries(
    ...
    *,
    ...
    return _validate_clusters(cast("list[Any]", clusters))  # pyright: ignore[reportAny]


def _validate_clusters(raw_clusters: list[Any]) -> list[dict[str, Any]]:
    """Keep only well-formed clusters from the model's JSON reply.

    A cluster is an object with a string ``topic_title`` and a ``file_paths``
    list of strings.  Anything else is dropped with a warning; membership of
    the paths in the indexed set is checked at render time.
    """
    valid: list[dict[str, Any]] = []
    for entry in raw_clusters:  # pyright: ignore[reportAny]
        if not isinstance(entry, dict):
            log.warning("dropping cluster: expected an object, got %s", type(entry).__name__)  # pyright: ignore[reportAny]
            continue
        cluster = cast("dict[str, Any]", entry)
        title: Any = cluster.get("topic_title")  # pyright: ignore[reportAny]
        file_paths: Any = cluster.get("file_paths")  # pyright: ignore[reportAny]
        if not isinstance(title, str):
            log.warning("dropping cluster: topic_title is not a string")
            continue
        if not isinstance(file_paths, list) or not all(
            isinstance(fp, str)
            for fp in file_paths  # pyright: ignore[reportUnknownVariableType]
        ):
            log.warning("dropping cluster %r: file_paths is not a list of strings", title)
            continue
        valid.append({"topic_title": title, "file_paths": list(file_paths)})  # pyright: ignore[reportUnknownArgumentType]
    return valid
...
_PLAIN_LINE_ESCAPE_RE = re.compile(r"([\\\[\]()!<`{#*_~])")


def _plain_line(text: str) -> str:
    """Reduce model-supplied *text* to one line of literal Markdown text.

    Whitespace runs (including newlines) collapse to a single space, and the
    characters that could open a link, image, fence, raw HTML, macro, heading,
    emphasis or strikethrough are backslash-escaped so the text renders as
    written.  Parentheses are escaped too, so ``](`` can never appear: the
    attachment scanner matches link syntax by regex, not by parsing.
    """
    collapsed = " ".join(text.split())
    return _PLAIN_LINE_ESCAPE_RE.sub(r"\\\1", collapsed)
...
def _render_summary_bullet(rel_path: str, summary: str) -> str:
    ...
    return f"- **[{title}]({rel_path})** — {_plain_line(summary)}\n"
...
def _render_cluster_section(
    ...
    Only paths present in *summary_map* (the indexed files) are rendered; any
    other path the model named is skipped with a warning.  Returns
    ``("", set())`` for clusters with no usable file_paths (skipped).
    ...
    heading = _plain_line(str(topic_title)) or "Other"  # pyright: ignore[reportAny]
    bullets: list[str] = []
    claimed: set[str] = set()
    for fp_raw in file_paths:  # pyright: ignore[reportUnknownVariableType]
        if not isinstance(fp_raw, str):
            continue
        summary = summary_map.get(fp_raw)
        if summary is None:
            log.warning(
                "cluster %r names %r, which is not an indexed file; skipped", heading, fp_raw
            )
            continue
        bullets.append(_render_summary_bullet(fp_raw, summary))
        claimed.add(fp_raw)
    if not bullets:
        return "", set()
    return f"\n## {heading}\n\n" + "".join(bullets), claimed
