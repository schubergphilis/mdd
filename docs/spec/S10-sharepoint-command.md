...
rename) that refuse to write through a symlink at the destination, at
its `*.tmp` sibling, or at any directory between `<output>/` and the
destination (the file counts as an error). Incremental by default — skip if destination is newer than
