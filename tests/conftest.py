...
work without each test attaching a handler manually.
...
def _mdd_default_backend() -> None:  # pyright: ignore[reportUnusedFunction]
    ...
    entry point has wired one (a stray resolution must be a loud error,
    never a silent fall-through to an unguarded backend).
