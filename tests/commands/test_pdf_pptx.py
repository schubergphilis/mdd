...
from mdd.commands.pdf_pptx import PPTX_APPLESCRIPT, export_pptx_to_pdf
...
class TestAppleScriptArguments:
    ...
    def test_control_character_in_name_is_skipped(
        ...
        (tmp_path / "bad\nname.pptx").write_bytes(b"fake")
        (tmp_path / "good.pptx").write_bytes(b"fake")

        with patch("mdd.commands.pdf_pptx.export_pptx_to_pdf", return_value=True) as mock:
            result = main(["pdf-pptx", str(tmp_path)])

        assert result == 0
        names = [call.args[0].name for call in mock.call_args_list]
        assert names == ["good.pptx"]
