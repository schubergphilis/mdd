"""Tests for mdd.ai.rewrite — protected-region pipeline and rewrite logic."""

from __future__ import annotations

import hashlib
from pathlib import Path
from typing import Any, cast
from unittest.mock import MagicMock

import pytest

from mdd.ai.models import ChatResult
from mdd.ai.rewrite import (
    _CHUNK_TARGET_CHARS as _CHUNK_TARGET_CHARS,  # pyright: ignore[reportPrivateUsage]
)
from mdd.ai.rewrite import (
    _MAX_OUTPUT_TOKENS as _MAX_OUTPUT_TOKENS,  # pyright: ignore[reportPrivateUsage]
)
from mdd.ai.rewrite import (
    _compose_system_prompt as _compose_system_prompt,  # pyright: ignore[reportPrivateUsage]
)
from mdd.ai.rewrite import (
    _default_tone_path as _default_tone_path,  # pyright: ignore[reportPrivateUsage]
)
from mdd.ai.rewrite import (
    _load_constraints as _load_constraints,  # pyright: ignore[reportPrivateUsage]
)
from mdd.ai.rewrite import _load_tone as _load_tone  # pyright: ignore[reportPrivateUsage]
from mdd.ai.rewrite import (
    _restore_boundary_whitespace as _restore_boundary_whitespace,  # pyright: ignore[reportPrivateUsage]
)
from mdd.ai.rewrite import (
    _split_frontmatter as _split_frontmatter,  # pyright: ignore[reportPrivateUsage]
)
from mdd.ai.rewrite import (
    extract_protected,
    rewrite_file,
    rewrite_files,
    split_into_chunks,
    stitch_protected,
)

# ---------------------------------------------------------------------------
# Fixtures
# ---------------------------------------------------------------------------

FIXTURES_DIR = Path(__file__).parent / "fixtures"


def _make_chat_result(text: str, cached: bool = False) -> ChatResult:
    return ChatResult(
        text=text,
        cached=cached,
        prompt_tokens=10,
        completion_tokens=5,
        cost_usd=None,
    )


def _paragraph(marker: str, words: int = 60) -> str:
    """Return one identifiable paragraph of prose."""
    return f"{marker} " + ("some sentence of ordinary prose. " * words) + "\n\n"


def _long_page(sections: int, paragraphs: int = 3) -> str:
    """Return a page of *sections* H2 sections, each well over the chunk target."""
    parts = ["# Title\n\nIntro prose paragraph.\n\n"]
    for s in range(sections):
        parts.append(f"## Section {s}\n\n")
        parts.extend(_paragraph(f"s{s}p{p}", 40) for p in range(paragraphs))
    return "".join(parts)


def _structural_body() -> str:
    """Return the hand-authored fixture holding the shapes that break a splitter."""
    return (FIXTURES_DIR / "long-structure.md").read_text(encoding="utf-8")


_SCALE_TOPICS = (
    "ingestion",
    "retention",
    "labelling",
    "migration",
    "reconciliation",
    "throttling",
    "provisioning",
    "auditing",
)


def _scale_section(index: int) -> str:
    """Return one section of the generated scale body, with varied heading text."""
    topic = _SCALE_TOPICS[index % len(_SCALE_TOPICS)]
    paragraphs = "".join(
        f"Paragraph {p} of the {topic} section, number {index}. "
        + ("A sentence of ordinary filler prose. " * 6)
        + "\n\n"
        for p in range(3)
    )
    table = (
        f"| Key {index} | Value |\n"
        "|--------|-------|\n"
        f"| {topic}-a | first placeholder value |\n"
        f"| {topic}-b | second placeholder value |\n"
        "\n"
    )
    fence = f"```text\nfenced sample for {topic} {index}\nsecond line of the block\n```\n\n"
    return (
        f"## Section {index}: {topic}\n\n"
        + paragraphs
        + f"### {topic.title()} detail {index}\n\n"
        + ("Detail prose that says nothing in particular. " * 8)
        + "\n\n"
        + table
        + fence
    )


def _scale_body(sections: int = 41) -> str:
    """Return a body of roughly 60k characters, built rather than committed.

    A 60KB blob in the tree would be noise no reader benefits from, and it
    would have to be re-generated by hand every time the shapes change.
    """
    return (
        "# Generated Scale Page\n\nIntroductory prose before the first section.\n\n"
        + "".join(_scale_section(i) for i in range(sections))
        + "> *Generated by mdd — https://github.com/schubergphilis/mdd*\n"
    )


def _identity_rewrite(chunks: list[str]) -> str:
    """Join *chunks* unchanged, standing in for a model that returns its input."""
    return "".join(chunks)


def _make_mock_client(return_text: str, cached: bool = False) -> MagicMock:
    """Return a mock Client whose chat() returns the given text."""
    mock = MagicMock()
    mock.chat.return_value = _make_chat_result(return_text, cached=cached)
    mock.summary.api_calls = 1
    mock.summary.prompt_tokens = 10
    mock.summary.completion_tokens = 5
    mock.summary.cost_usd = 0.0
    return mock


# ---------------------------------------------------------------------------
# extract_protected / stitch_protected
# ---------------------------------------------------------------------------


class TestProtectedRegionPipeline:
    def test_no_protected_regions(self) -> None:
        text = "Hello world.\n\nSome more text."
        transformed, regions = extract_protected(text)
        assert transformed == text
        assert regions == []

    def test_frontmatter_extracted(self) -> None:
        text = "---\ntitle: Test\n---\n\nBody text here."
        transformed, regions = extract_protected(text)
        assert "---\ntitle: Test\n---\n" in regions[0].original
        assert "__MDD_PROTECTED_0__" in transformed
        assert "Body text here." in transformed

    def test_code_block_extracted(self) -> None:
        text = "Before\n\n```python\nx = 1\n```\n\nAfter"
        transformed, _regions = extract_protected(text)
        assert "```python" not in transformed
        assert "__MDD_PROTECTED_0__" in transformed
        assert "Before" in transformed
        assert "After" in transformed

    def test_fenced_confluence_block_extracted(self) -> None:
        text = "Prose\n\n```{=confluence}\n<ac:macro/>\n```\n\nMore prose"
        transformed, regions = extract_protected(text)
        assert "{=confluence}" not in transformed
        assert len(regions) == 1
        assert "{=confluence}" in regions[0].original

    def test_fenced_html_block_extracted(self) -> None:
        text = "Prose\n\n```{=html}\n<div>test</div>\n```\n\nMore"
        transformed, regions = extract_protected(text)
        assert "{=html}" not in transformed
        assert len(regions) == 1

    def test_table_extracted(self) -> None:
        table = "| A | B |\n|---|---|\n| 1 | 2 |\n"
        text = f"Before\n\n{table}\nAfter"
        transformed, regions = extract_protected(text)
        assert "| A |" not in transformed
        assert len(regions) == 1
        assert "| A |" in regions[0].original

    def test_confluence_export_header_extracted(self) -> None:
        header = "> **Confluence export** — Auto-generated.\n> Source: https://example.com\n"
        text = f"{header}\n# Title\n\nBody"
        transformed, regions = extract_protected(text)
        assert "> **Confluence export**" not in transformed
        assert len(regions) == 1
        assert "> **Confluence export**" in regions[0].original

    def test_mdd_footer_extracted(self) -> None:
        footer = "> *Generated by mdd — https://github.com/schubergphilis/mdd*\n"
        text = f"Body text\n\n{footer}"
        transformed, regions = extract_protected(text)
        assert "> *Generated by mdd" not in transformed
        assert len(regions) == 1
        assert "> *Generated by mdd" in regions[0].original

    def test_multiple_protected_regions(self) -> None:
        text = "```python\nx=1\n```\n\nProse middle\n\n| A | B |\n|---|---|\n| 1 | 2 |\n\nEnd"
        transformed, regions = extract_protected(text)
        assert len(regions) == 2
        assert "__MDD_PROTECTED_0__" in transformed
        assert "__MDD_PROTECTED_1__" in transformed
        assert "Prose middle" in transformed
        assert "End" in transformed

    def test_round_trip_preserves_content(self) -> None:
        """Extract → uppercase prose (model stub) → stitch → identical protected content."""
        text = (
            "---\ntitle: Doc\n---\n\n"
            "This is prose.\n\n"
            "```python\nx = 42\n```\n\n"
            "More prose.\n\n"
            "| H1 | H2 |\n|----|----|\n| a  | b  |\n"
        )
        transformed, regions = extract_protected(text)

        # Simulate a model that uppercases all text (but leaves placeholders alone).
        def uppercase_non_placeholder(t: str) -> str:
            import re

            parts = re.split(r"(__MDD_PROTECTED_\d+__)", t)
            return "".join(p if p.startswith("__MDD_PROTECTED_") else p.upper() for p in parts)

        model_output = uppercase_non_placeholder(transformed)
        result = stitch_protected(model_output, regions)

        # Protected regions must be byte-identical.
        assert "---\ntitle: Doc\n---\n" in result
        assert "```python\nx = 42\n```" in result
        assert "| H1 | H2 |" in result

    def test_stitch_raises_on_missing_placeholder(self) -> None:
        text = "Prose\n\n```code\ncontent\n```\n\nMore"
        _, regions = extract_protected(text)
        # Simulate model that dropped the placeholder
        bad_output = "PROSE\n\nMORE"
        with pytest.raises(ValueError, match="missing from the model"):
            stitch_protected(bad_output, regions)

    def test_full_protected_fixture(self) -> None:
        """All protected types in the fixture file round-trip correctly."""
        content = (FIXTURES_DIR / "protected.md").read_text(encoding="utf-8")
        transformed, regions = extract_protected(content)

        # Simulate a model that returns the transformed text as-is (identity).
        result = stitch_protected(transformed, regions)
        assert result == content

    def test_tilde_fenced_block_extracted(self) -> None:
        text = "Before\n\n~~~\nsome content\n~~~\n\nAfter"
        transformed, regions = extract_protected(text)
        assert "~~~" not in transformed
        assert len(regions) == 1


# ---------------------------------------------------------------------------
# split_into_chunks
# ---------------------------------------------------------------------------


class TestSplitIntoChunks:
    def test_short_text_is_one_chunk(self) -> None:
        text = "# Title\n\nShort prose.\n"
        assert split_into_chunks(text) == [text]

    def test_empty_text_round_trips(self) -> None:
        assert "".join(split_into_chunks("")) == ""

    def test_join_is_byte_identical(self) -> None:
        text = _long_page(8)
        chunks = split_into_chunks(text)
        assert len(chunks) > 1
        assert "".join(chunks) == text

    def test_join_is_byte_identical_on_corpus_fixture(self) -> None:
        """The invariant must hold on real markdown, at every target size."""
        content = (FIXTURES_DIR / "protected.md").read_text(encoding="utf-8")
        transformed, _regions = extract_protected(content)
        for target in (10, 40, 200, 1000, _CHUNK_TARGET_CHARS):
            chunks = split_into_chunks(transformed, target)
            assert "".join(chunks) == transformed, f"join broken at target={target}"

    def test_splits_on_h2_boundaries(self) -> None:
        text = _long_page(4)
        chunks = split_into_chunks(text)
        # Every chunk after the first starts at a heading, not mid-section.
        for chunk in chunks[1:]:
            assert chunk.startswith("## "), chunk[:40]

    def test_falls_back_to_h3_within_one_h2_section(self) -> None:
        body = "".join(f"### Sub {i}\n\n{_paragraph(f'x{i}', 40)}" for i in range(12))
        text = "## Only Section\n\n" + body
        chunks = split_into_chunks(text)
        assert len(chunks) > 1
        for chunk in chunks[1:]:
            assert chunk.startswith("### "), chunk[:40]

    def test_falls_back_to_paragraphs_within_one_section(self) -> None:
        text = "## Only Section\n\n" + "".join(_paragraph(f"p{i}", 40) for i in range(12))
        chunks = split_into_chunks(text)
        assert len(chunks) > 1
        assert "".join(chunks) == text
        # No chunk starts mid-paragraph: each one begins with a paragraph marker.
        for chunk in chunks[1:]:
            assert chunk.startswith("p"), chunk[:20]

    def test_falls_back_to_lines_within_one_paragraph(self) -> None:
        # One paragraph, no blank lines: only hard line breaks are available.
        text = "\n".join(f"line {i} " + ("word " * 40) for i in range(80))
        chunks = split_into_chunks(text)
        assert len(chunks) > 1
        assert "".join(chunks) == text
        for chunk in chunks:
            assert len(chunk) <= _CHUNK_TARGET_CHARS

    def test_one_enormous_unbroken_line_is_emitted_whole(self) -> None:
        """A page that is a single line has no boundary to split on."""
        text = "word " * 5000
        chunks = split_into_chunks(text)
        assert chunks == [text]

    def test_chunks_stay_under_target_when_boundaries_allow(self) -> None:
        chunks = split_into_chunks(_long_page(8))
        assert all(len(chunk) <= _CHUNK_TARGET_CHARS for chunk in chunks)

    def test_small_sections_are_packed_together(self) -> None:
        """Boundaries are opportunities, not mandates — do not emit tiny chunks."""
        text = "".join(f"## S{i}\n\nA little prose.\n\n" for i in range(20))
        assert split_into_chunks(text) == [text]


class TestSplitStructuralFixture:
    """The hand-authored fixture carries the shapes that break a naive splitter.

    Driving it at a small target exercises the same code paths a genuinely huge
    page would, without committing a huge page.
    """

    def test_join_is_byte_identical_at_several_targets(self) -> None:
        transformed, _regions = extract_protected(_structural_body())
        for target in (200, 500, 2000):
            chunks = split_into_chunks(transformed, target)
            assert len(chunks) > 1, f"expected a split at target={target}"
            assert "".join(chunks) == transformed, f"join broken at target={target}"

    def test_no_placeholder_is_split_across_chunks(self) -> None:
        transformed, regions = extract_protected(_structural_body())
        assert len(regions) > 1
        for target in (200, 500, 2000):
            chunks = split_into_chunks(transformed, target)
            for region in regions:
                hits = sum(region.placeholder in chunk for chunk in chunks)
                assert hits == 1, f"{region.placeholder} in {hits} chunks at target={target}"

    def test_unbroken_paragraph_is_the_only_over_target_chunk(self) -> None:
        """A single line longer than the target is emitted whole, by design."""
        target = 500
        transformed, _regions = extract_protected(_structural_body())
        oversized = [
            chunk for chunk in split_into_chunks(transformed, target) if len(chunk) > target
        ]
        assert len(oversized) == 1
        # It really is one line: there was no interior boundary left to use.
        assert "\n" not in oversized[0].rstrip("\n")
        assert "one physical line" in oversized[0]

    def test_table_larger_than_the_target_survives_in_one_chunk(self) -> None:
        """Protection, not the boundary hierarchy, is what keeps a big table whole."""
        target = 500
        transformed, regions = extract_protected(_structural_body())
        tables = [r for r in regions if r.kind == "table" and len(r.original) > target]
        assert tables, "fixture must hold a table bigger than the small target"
        chunks = split_into_chunks(transformed, target)
        for table in tables:
            holders = [chunk for chunk in chunks if table.placeholder in chunk]
            assert len(holders) == 1
            assert table.original in stitch_protected(holders[0], [table])


class TestSplitAtScale:
    """A body around 60k characters, at the production target."""

    def test_body_is_around_sixty_thousand_chars(self) -> None:
        assert 55_000 <= len(_scale_body()) <= 70_000

    def test_join_is_byte_identical(self) -> None:
        body = _scale_body()
        chunks = split_into_chunks(body)
        assert "".join(chunks) == body

    def test_chunk_count_is_plausible(self) -> None:
        """A range, not an exact number: the packing heuristic is free to change."""
        transformed, _regions = extract_protected(_scale_body())
        lower = len(transformed) // _CHUNK_TARGET_CHARS
        chunks = split_into_chunks(transformed)
        assert lower <= len(chunks) <= lower * 3

    def test_chunks_respect_the_target_except_single_long_lines(self) -> None:
        transformed, _regions = extract_protected(_scale_body())
        for chunk in split_into_chunks(transformed):
            if len(chunk) <= _CHUNK_TARGET_CHARS:
                continue
            assert "\n" not in chunk.rstrip("\n"), "over-target chunk had a boundary to use"

    def test_every_placeholder_lands_in_exactly_one_chunk(self) -> None:
        transformed, regions = extract_protected(_scale_body())
        chunks = split_into_chunks(transformed)
        assert len(chunks) > 1
        assert len(regions) > 1
        for region in regions:
            hits = sum(region.placeholder in chunk for chunk in chunks)
            assert hits == 1, f"{region.placeholder} appears in {hits} chunks"

    def test_extract_split_identity_join_stitch_round_trips(self) -> None:
        """The whole pipeline, with a model that returns each chunk unchanged."""
        body = _scale_body()
        transformed, regions = extract_protected(body)
        chunks = split_into_chunks(transformed)
        assert len(chunks) > 1
        assert stitch_protected(_identity_rewrite(chunks), regions) == body


# ---------------------------------------------------------------------------
# _split_frontmatter
# ---------------------------------------------------------------------------


class TestSplitFrontmatter:
    def test_no_frontmatter(self) -> None:
        body = "# Title\n\nContent"
        fm, b = _split_frontmatter(body)
        assert fm == ""
        assert b == body

    def test_with_frontmatter(self) -> None:
        text = "---\ntitle: Test\n---\n\n# Body"
        fm, b = _split_frontmatter(text)
        assert fm == "---\ntitle: Test\n---\n"
        assert b == "\n# Body"

    def test_frontmatter_not_sent_to_model(self) -> None:
        """The frontmatter block must NOT appear in the body sent to the model."""
        text = "---\ntitle: Secret\nauthor: Alice\n---\n\nBody here."
        _fm, b = _split_frontmatter(text)
        assert "title: Secret" not in b
        assert "Body here." in b


# ---------------------------------------------------------------------------
# _load_tone / _load_constraints / _compose_system_prompt
# ---------------------------------------------------------------------------


class TestLoadTone:
    def test_default_tone_exists(self) -> None:
        path = _default_tone_path()
        assert path.exists(), f"Default tone prompt not found: {path}"

    def test_default_tone_returns_digest(self) -> None:
        text, digest = _load_tone(None)
        assert isinstance(text, str)
        assert len(text) > 0
        assert isinstance(digest, bytes)
        assert len(digest) == 32  # SHA-256

    def test_custom_tone(self, tmp_path: Path) -> None:
        style_file = tmp_path / "my-style.md"
        style_file.write_text("Be very concise.", encoding="utf-8")
        text, digest = _load_tone(style_file)
        assert text == "Be very concise."
        expected_digest = hashlib.sha256(b"Be very concise.").digest()
        assert digest == expected_digest

    def test_different_tones_produce_different_digests(self, tmp_path: Path) -> None:
        a = tmp_path / "a.md"
        b = tmp_path / "b.md"
        a.write_text("Style A", encoding="utf-8")
        b.write_text("Style B", encoding="utf-8")
        _, da = _load_tone(a)
        _, db = _load_tone(b)
        assert da != db


class TestLoadConstraints:
    def test_constraints_file_exists(self) -> None:
        text, digest = _load_constraints()
        assert isinstance(text, str)
        assert len(text) > 0
        assert isinstance(digest, bytes)
        assert len(digest) == 32

    def test_constraints_mention_placeholder_preservation(self) -> None:
        text, _ = _load_constraints()
        # The load-bearing instruction must be present — this is the
        # whole point of splitting the prompt: --style cannot drop it.
        assert "placeholder" in text.lower()


class TestComposeSystemPrompt:
    def test_combines_tone_and_constraints(self) -> None:
        composed = _compose_system_prompt("TONE_BLOCK", "CONSTRAINTS_BLOCK")
        assert "TONE_BLOCK" in composed
        assert "CONSTRAINTS_BLOCK" in composed
        # Constraints come AFTER tone so they're the trailing instructions.
        assert composed.index("TONE_BLOCK") < composed.index("CONSTRAINTS_BLOCK")

    def test_custom_tone_preserves_bundled_constraints(self) -> None:
        constraints_text, _ = _load_constraints()
        composed = _compose_system_prompt("Be terse.", constraints_text)
        # The user-supplied tone cannot drop the placeholder rule.
        assert "placeholder" in composed.lower()


# ---------------------------------------------------------------------------
# rewrite_file
# ---------------------------------------------------------------------------


class TestRewriteFile:
    def test_produces_sibling_by_default(self, tmp_path: Path) -> None:
        src = tmp_path / "page.md"
        src.write_text("# Hello\n\nSome text.\n", encoding="utf-8")

        mock_client = _make_mock_client("# Hello\n\nRewritten text.\n")

        result = rewrite_file(src, mock_client)  # pyright: ignore[reportArgumentType]

        assert result.status == "rewritten"
        assert result.output_path == tmp_path / "page.md.rewrite.md"
        assert result.output_path is not None
        assert result.output_path.exists()
        assert "Rewritten text." in result.output_path.read_text()

    def test_apply_overwrites_in_place(self, tmp_path: Path) -> None:
        src = tmp_path / "page.md"
        src.write_text("# Hello\n\nOriginal text.\n", encoding="utf-8")

        mock_client = _make_mock_client("# Hello\n\nApplied text.\n")

        result = rewrite_file(src, mock_client, apply=True)  # pyright: ignore[reportArgumentType]

        assert result.status == "rewritten"
        assert result.output_path == src
        assert "Applied text." in src.read_text()

    def test_frontmatter_preserved_byte_identical(self, tmp_path: Path) -> None:
        fm = "---\ntitle: Test\nconfluence:\n  page_id: '123'\n---\n"
        body = "\n# Body\n\nSome prose.\n"
        src = tmp_path / "page.md"
        src.write_text(fm + body, encoding="utf-8")

        # Model returns only the body (no frontmatter in model output).
        mock_client = _make_mock_client("# Body\n\nRewritten prose.\n")

        result = rewrite_file(src, mock_client, apply=True)  # pyright: ignore[reportArgumentType]

        assert result.status == "rewritten"
        content = src.read_text()
        assert content.startswith(fm)

    def test_boundary_whitespace_restored(self, tmp_path: Path) -> None:
        # Source has a blank line after the frontmatter and trailing blank lines.
        fm = "---\ntitle: Test\n---\n"
        body = "\n> Header\n\n# Title\n\n{{confluence:children}}\n\n  \n\n"
        src = tmp_path / "page.md"
        src.write_text(fm + body, encoding="utf-8")

        # Model trims the leading blank line and all trailing whitespace.
        mock_client = _make_mock_client("> Header\n\n# Title\n\n{{confluence:children}}")

        result = rewrite_file(src, mock_client, apply=True)  # pyright: ignore[reportArgumentType]

        assert result.status == "rewritten"
        # Boundary whitespace matches the source exactly — no churn.
        assert src.read_text(encoding="utf-8") == fm + body

    def test_whitespace_only_body_is_skipped_untouched(self, tmp_path: Path) -> None:
        body = "\n\n  \n"
        src = tmp_path / "page.md"
        src.write_text(body, encoding="utf-8")

        mock_client = _make_mock_client("   \n")

        result = rewrite_file(src, mock_client, apply=True)  # pyright: ignore[reportArgumentType]

        assert result.status == "skipped"
        assert src.read_text(encoding="utf-8") == body

    def test_whitespace_only_model_output_keeps_source_body(self) -> None:
        """A model that returns only whitespace must not blank the body."""
        assert _restore_boundary_whitespace("\n\n  \n", "   \n") == "\n\n  \n"

    def test_missing_placeholder_returns_error(self, tmp_path: Path) -> None:
        src = tmp_path / "page.md"
        src.write_text("Some prose.\n\n```python\nx=1\n```\n\nMore prose.\n", encoding="utf-8")

        # Model drops the placeholder.
        mock_client = _make_mock_client("Some prose. More prose.")

        result = rewrite_file(src, mock_client)  # pyright: ignore[reportArgumentType]

        assert result.status == "error"
        assert "missing" in (result.error or "").lower()

    def test_missing_placeholder_dumps_model_output(self, tmp_path: Path) -> None:
        src = tmp_path / "page.md"
        src.write_text("Some prose.\n\n```python\nx=1\n```\n\nMore prose.\n", encoding="utf-8")

        mock_client = _make_mock_client("Some prose. More prose.")

        result = rewrite_file(src, mock_client)  # pyright: ignore[reportArgumentType]

        assert result.fail_path == tmp_path / "page.md.rewrite.fail"
        assert result.fail_path is not None
        dump = result.fail_path.read_text(encoding="utf-8")
        # Header carries the diagnosis; the raw model output follows verbatim.
        assert "kind=fenced" in dump
        assert "present=False" in dump
        assert dump.endswith("Some prose. More prose.")
        # The dump must not look like a markdown page to the next sweep.
        assert result.fail_path.suffix != ".md"

    def test_truncated_completion_refused(self, tmp_path: Path) -> None:
        src = tmp_path / "page.md"
        src.write_text("# Hello\n\nSome text.\n", encoding="utf-8")

        mock_client = _make_mock_client("# Hello\n\nSome tex")
        mock_client.chat.return_value = ChatResult(
            text="# Hello\n\nSome tex",
            cached=False,
            prompt_tokens=10,
            completion_tokens=5,
            finish_reason="length",
            cost_usd=None,
        )

        result = rewrite_file(src, mock_client, apply=True)  # pyright: ignore[reportArgumentType]

        assert result.status == "error"
        assert "finish_reason='length'" in (result.error or "")
        # The source is untouched and the partial output is available for review.
        assert src.read_text(encoding="utf-8") == "# Hello\n\nSome text.\n"
        assert result.fail_path is not None
        assert result.fail_path.read_text(encoding="utf-8").endswith("# Hello\n\nSome tex")

    def test_large_shrink_warns_but_still_writes(
        self, tmp_path: Path, caplog: pytest.LogCaptureFixture
    ) -> None:
        src = tmp_path / "page.md"
        src.write_text("# Hello\n\n" + ("Some sentence of prose. " * 40) + "\n", encoding="utf-8")

        mock_client = _make_mock_client("# Hello\n\nSome sentence of prose.\n")

        with caplog.at_level("WARNING", logger="mdd.ai.rewrite"):
            result = rewrite_file(src, mock_client, apply=True)  # pyright: ignore[reportArgumentType]

        assert result.status == "rewritten"
        assert "shrank" in caplog.text

    def test_stub_page_skipped_without_api_call(self, tmp_path: Path) -> None:
        """A heading plus a macro has nothing to rewrite — do not spend a call on it."""
        src = tmp_path / "page.md"
        body = (
            "\n> **Confluence export**\n>\n> Exported from confluence.\n\n"
            '# 02. Recent Changes\n\n{{confluence:recently-updated max="100"}}\n'
        )
        src.write_text(body, encoding="utf-8")

        mock_client = _make_mock_client("whatever")

        result = rewrite_file(src, mock_client, apply=True)  # pyright: ignore[reportArgumentType]

        assert result.status == "skipped"
        mock_client.chat.assert_not_called()
        assert src.read_text(encoding="utf-8") == body

    def test_one_prose_line_is_enough_to_rewrite(self, tmp_path: Path) -> None:
        """The skip is about *kind* of content, not length — short prose still counts."""
        src = tmp_path / "page.md"
        src.write_text("# Title\n\nSome text.\n", encoding="utf-8")

        mock_client = _make_mock_client("# Title\n\nRewritten text.\n")

        result = rewrite_file(src, mock_client, apply=True)  # pyright: ignore[reportArgumentType]

        assert result.status == "rewritten"

    def test_max_tokens_is_the_flat_cap(self, tmp_path: Path) -> None:
        """The gateway default (4096) truncates long pages, so ask for the cap."""
        src = tmp_path / "page.md"
        src.write_text("# Title\n\n" + ("Some sentence of prose. " * 20), encoding="utf-8")

        mock_client = _make_mock_client("# Title\n\nShort.\n")

        _ = rewrite_file(src, mock_client)  # pyright: ignore[reportArgumentType]

        assert cast("int", mock_client.chat.call_args.kwargs["max_tokens"]) == _MAX_OUTPUT_TOKENS

    def test_max_tokens_does_not_depend_on_page_length(self, tmp_path: Path) -> None:
        """Every chunk asks for the same ceiling; nothing is estimated per page."""
        src = tmp_path / "page.md"
        src.write_text(_long_page(6), encoding="utf-8")

        mock_client = _make_mock_client("Rewritten.\n")

        _ = rewrite_file(src, mock_client)  # pyright: ignore[reportArgumentType]

        assert mock_client.chat.call_count > 1
        for call in mock_client.chat.call_args_list:
            assert cast("int", call.kwargs["max_tokens"]) == _MAX_OUTPUT_TOKENS

    def test_single_chunk_page_makes_exactly_one_call(self, tmp_path: Path) -> None:
        src = tmp_path / "page.md"
        src.write_text("# Title\n\nSome prose here.\n", encoding="utf-8")

        mock_client = _make_mock_client("# Title\n\nRewritten prose.\n")

        result = rewrite_file(src, mock_client)  # pyright: ignore[reportArgumentType]

        assert result.status == "rewritten"
        assert mock_client.chat.call_count == 1

    def test_cache_hit_returns_cached_status(self, tmp_path: Path) -> None:
        src = tmp_path / "page.md"
        src.write_text("# Hello\n\nSome text.\n", encoding="utf-8")

        mock_client = _make_mock_client("# Hello\n\nCached text.\n", cached=True)

        result = rewrite_file(src, mock_client)  # pyright: ignore[reportArgumentType]

        assert result.status == "cached"

    def test_file_not_found_returns_error(self, tmp_path: Path) -> None:
        src = tmp_path / "nonexistent.md"
        mock_client = MagicMock()

        result = rewrite_file(src, mock_client)  # pyright: ignore[reportArgumentType]

        assert result.status == "error"
        assert "not found" in (result.error or "").lower()

    def test_style_hash_used_in_cache_key(self, tmp_path: Path) -> None:
        """Different style files must produce different cache keys."""
        src = tmp_path / "page.md"
        src.write_text("# Hello\n\nText.\n", encoding="utf-8")

        style_a = tmp_path / "style_a.md"
        style_b = tmp_path / "style_b.md"
        style_a.write_text("Style A", encoding="utf-8")
        style_b.write_text("Style B", encoding="utf-8")

        call_extras: list[bytes] = []

        def capture_chat(**kwargs: Any) -> ChatResult:  # pyright: ignore[reportAny]
            call_extras.append(kwargs.get("cache_key_extra", b""))
            return _make_chat_result("# Hello\n\nResult.\n")

        mock_client = MagicMock()
        mock_client.chat.side_effect = capture_chat
        mock_client.summary.api_calls = 0
        mock_client.summary.prompt_tokens = 0
        mock_client.summary.completion_tokens = 0
        mock_client.summary.cost_usd = 0.0

        rewrite_file(src, mock_client, style_path=style_a)  # pyright: ignore[reportArgumentType]
        rewrite_file(src, mock_client, style_path=style_b)  # pyright: ignore[reportArgumentType]

        assert len(call_extras) == 2
        assert call_extras[0] != call_extras[1]

    def test_code_block_preserved_through_rewrite(self, tmp_path: Path) -> None:
        """Protected code blocks must be byte-identical after rewrite."""
        code_block = "```python\ndef my_func():\n    return 42\n```"
        src = tmp_path / "page.md"
        src.write_text(f"Intro prose.\n\n{code_block}\n\nOutro prose.\n", encoding="utf-8")

        # Model returns transformed prose with placeholder preserved
        def identity_model(**kwargs: Any) -> ChatResult:  # pyright: ignore[reportAny]
            user_text: str = kwargs["user"]
            # Replace non-placeholder text with uppercase, leave placeholder.
            import re

            parts = re.split(r"(__MDD_PROTECTED_\d+__)", user_text)
            out = "".join(p if p.startswith("__MDD_PROTECTED_") else p.upper() for p in parts)
            return _make_chat_result(out)

        mock_client = MagicMock()
        mock_client.chat.side_effect = identity_model
        mock_client.summary.api_calls = 1
        mock_client.summary.prompt_tokens = 10
        mock_client.summary.completion_tokens = 5
        mock_client.summary.cost_usd = 0.0

        result = rewrite_file(src, mock_client, apply=True)  # pyright: ignore[reportArgumentType]

        assert result.status == "rewritten"
        content = src.read_text()
        assert code_block in content

    def test_table_preserved_through_rewrite(self, tmp_path: Path) -> None:
        table = "| A | B |\n|---|---|\n| 1 | 2 |\n"
        src = tmp_path / "page.md"
        src.write_text(f"Intro.\n\n{table}\nOutro.\n", encoding="utf-8")

        def identity_model(**kwargs: Any) -> ChatResult:  # pyright: ignore[reportAny]
            user_text: str = kwargs["user"]
            import re

            parts = re.split(r"(__MDD_PROTECTED_\d+__)", user_text)
            out = "".join(p if p.startswith("__MDD_PROTECTED_") else p.upper() for p in parts)
            return _make_chat_result(out)

        mock_client = MagicMock()
        mock_client.chat.side_effect = identity_model
        mock_client.summary.api_calls = 1
        mock_client.summary.prompt_tokens = 10
        mock_client.summary.completion_tokens = 5
        mock_client.summary.cost_usd = 0.0

        result = rewrite_file(src, mock_client, apply=True)  # pyright: ignore[reportArgumentType]

        assert result.status == "rewritten"
        content = src.read_text()
        assert table in content


class TestRewriteChunkedPage:
    """A page too long for one completion is rewritten chunk by chunk."""

    @staticmethod
    def _echo_client(
        *, truncate_call: int | None = None, cached: bool = False
    ) -> tuple[MagicMock, list[str]]:
        """Return a client that echoes each chunk back, and the list of chunks sent."""
        sent: list[str] = []

        def echo(**kwargs: Any) -> ChatResult:  # pyright: ignore[reportAny]
            user: str = kwargs["user"]
            sent.append(user)
            truncated = truncate_call is not None and len(sent) == truncate_call
            return ChatResult(
                text=user.upper() if not truncated else user[:20].upper(),
                cached=cached,
                prompt_tokens=10,
                completion_tokens=5,
                cost_usd=0.5,
                finish_reason="length" if truncated else "stop",
            )

        mock = MagicMock()
        mock.chat.side_effect = echo
        mock.summary.api_calls = 1
        mock.summary.prompt_tokens = 10
        mock.summary.completion_tokens = 5
        mock.summary.cost_usd = 0.0
        return mock, sent

    def test_long_page_is_split_across_calls(self, tmp_path: Path) -> None:
        src = tmp_path / "page.md"
        body = _long_page(8)
        src.write_text(body, encoding="utf-8")

        mock_client, sent = self._echo_client()

        result = rewrite_file(src, mock_client, apply=True)  # pyright: ignore[reportArgumentType]

        assert result.status == "rewritten"
        assert len(sent) > 1
        # Nothing was lost or duplicated at the joins.
        assert "".join(sent) == body
        assert src.read_text(encoding="utf-8") == body.upper()

    def test_chunk_token_and_cost_totals_are_summed(self, tmp_path: Path) -> None:
        src = tmp_path / "page.md"
        src.write_text(_long_page(8), encoding="utf-8")

        mock_client, sent = self._echo_client()

        result = rewrite_file(src, mock_client)  # pyright: ignore[reportArgumentType]

        assert result.prompt_tokens == 10 * len(sent)
        assert result.completion_tokens == 5 * len(sent)
        assert result.cost_usd == pytest.approx(0.5 * len(sent))

    def test_each_chunk_gets_its_own_cache_key(self, tmp_path: Path) -> None:
        src = tmp_path / "page.md"
        src.write_text(_long_page(8), encoding="utf-8")

        mock_client, sent = self._echo_client()

        rewrite_file(src, mock_client)  # pyright: ignore[reportArgumentType]

        keys = [cast("bytes", c.kwargs["cache_key_extra"]) for c in mock_client.chat.call_args_list]
        assert len(keys) == len(sent)
        assert len(set(keys)) == len(keys)
        # The key is the chunk digest followed by the tone and constraints digests.
        assert keys[0].startswith(hashlib.sha256(sent[0].encode()).digest())

    def test_all_chunks_cached_reports_cached(self, tmp_path: Path) -> None:
        src = tmp_path / "page.md"
        src.write_text(_long_page(8), encoding="utf-8")

        mock_client, _sent = self._echo_client(cached=True)

        result = rewrite_file(src, mock_client)  # pyright: ignore[reportArgumentType]

        assert result.status == "cached"

    def test_one_uncached_chunk_reports_rewritten(self, tmp_path: Path) -> None:
        src = tmp_path / "page.md"
        src.write_text(_long_page(8), encoding="utf-8")

        calls: list[str] = []

        def mixed(**kwargs: Any) -> ChatResult:  # pyright: ignore[reportAny]
            user: str = kwargs["user"]
            calls.append(user)
            return ChatResult(
                text=user.upper(),
                cached=len(calls) > 1,  # only the first chunk missed the cache
                prompt_tokens=10,
                completion_tokens=5,
                cost_usd=None,
                finish_reason="stop",
            )

        mock_client = MagicMock()
        mock_client.chat.side_effect = mixed

        result = rewrite_file(src, mock_client)  # pyright: ignore[reportArgumentType]

        assert len(calls) > 1
        assert result.status == "rewritten"

    def test_protected_regions_survive_chunking(self, tmp_path: Path) -> None:
        table = "| A | B |\n|---|---|\n| 1 | 2 |\n"
        code = "```python\ndef f():\n    return 42\n```"
        body = _long_page(4) + f"## Tail\n\n{table}\n{code}\n\n" + _long_page(4)
        src = tmp_path / "page.md"
        src.write_text(body, encoding="utf-8")

        mock_client, sent = self._echo_client()

        result = rewrite_file(src, mock_client, apply=True)  # pyright: ignore[reportArgumentType]

        assert result.status == "rewritten"
        assert len(sent) > 1
        # No chunk boundary fell inside a protected block, and both blocks came
        # back byte-identical rather than upper-cased.
        content = src.read_text(encoding="utf-8")
        assert table in content
        assert code in content

    def test_truncated_chunk_refuses_the_whole_file(self, tmp_path: Path) -> None:
        src = tmp_path / "page.md"
        body = _long_page(8)
        src.write_text(body, encoding="utf-8")

        mock_client, sent = self._echo_client(truncate_call=2)

        result = rewrite_file(src, mock_client, apply=True)  # pyright: ignore[reportArgumentType]

        assert result.status == "error"
        assert "finish_reason='length'" in (result.error or "")
        # Nothing was written: a file half in the new tone is worse than a failure.
        assert src.read_text(encoding="utf-8") == body
        # And no chunk after the failing one was paid for.
        assert len(sent) == 2

    def test_truncation_dump_names_the_failing_chunk(self, tmp_path: Path) -> None:
        src = tmp_path / "page.md"
        src.write_text(_long_page(8), encoding="utf-8")

        mock_client, _sent = self._echo_client(truncate_call=2)

        result = rewrite_file(src, mock_client)  # pyright: ignore[reportArgumentType]

        assert result.fail_path is not None
        dump = result.fail_path.read_text(encoding="utf-8")
        total = len(split_into_chunks(extract_protected(_long_page(8))[0]))
        assert f"chunk 2 of {total}" in dump
        assert f"chunk 2 of {total}" in (result.error or "")

    def test_split_is_logged(self, tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
        src = tmp_path / "page.md"
        src.write_text(_long_page(8), encoding="utf-8")

        mock_client, _sent = self._echo_client()

        with caplog.at_level("INFO", logger="mdd.ai.rewrite"):
            rewrite_file(src, mock_client)  # pyright: ignore[reportArgumentType]

        assert "split into" in caplog.text

    def test_model_trimming_a_chunk_does_not_weld_lines(self, tmp_path: Path) -> None:
        """A chunk returned without its trailing newline must not join the next."""
        src = tmp_path / "page.md"
        body = _long_page(8)
        src.write_text(body, encoding="utf-8")

        def trimming(**kwargs: Any) -> ChatResult:  # pyright: ignore[reportAny]
            user: str = kwargs["user"]
            return ChatResult(
                text=user.strip(),
                cached=False,
                prompt_tokens=10,
                completion_tokens=5,
                cost_usd=None,
                finish_reason="stop",
            )

        mock_client = MagicMock()
        mock_client.chat.side_effect = trimming

        result = rewrite_file(src, mock_client, apply=True)  # pyright: ignore[reportArgumentType]

        assert result.status == "rewritten"
        assert src.read_text(encoding="utf-8") == body

    def test_scale_page_makes_one_call_per_chunk(self, tmp_path: Path) -> None:
        body = _scale_body()
        src = tmp_path / "page.md"
        src.write_text(body, encoding="utf-8")

        expected_chunks = len(split_into_chunks(extract_protected(body)[0]))
        assert expected_chunks > 1

        mock_client, sent = self._echo_client()

        result = rewrite_file(src, mock_client, apply=True)  # pyright: ignore[reportArgumentType]

        assert result.status == "rewritten"
        assert cast("int", mock_client.chat.call_count) == expected_chunks
        assert len(sent) == expected_chunks
        assert "".join(sent) == extract_protected(body)[0]

        calls = mock_client.chat.call_args_list
        keys = [cast("bytes", call.kwargs["cache_key_extra"]) for call in calls]
        assert len(set(keys)) == expected_chunks
        assert all(cast("int", call.kwargs["max_tokens"]) == _MAX_OUTPUT_TOKENS for call in calls)

    def test_client_error_mid_run_returns_error(self, tmp_path: Path) -> None:
        src = tmp_path / "page.md"
        body = _long_page(8)
        src.write_text(body, encoding="utf-8")

        calls: list[str] = []

        def boom(**kwargs: Any) -> ChatResult:  # pyright: ignore[reportAny]
            calls.append(cast("str", kwargs["user"]))
            if len(calls) == 2:
                raise RuntimeError("gateway exploded")
            return _make_chat_result(cast("str", kwargs["user"]))

        mock_client = MagicMock()
        mock_client.chat.side_effect = boom

        result = rewrite_file(src, mock_client, apply=True)  # pyright: ignore[reportArgumentType]

        assert result.status == "error"
        assert "gateway exploded" in (result.error or "")
        assert src.read_text(encoding="utf-8") == body


# ---------------------------------------------------------------------------
# rewrite_files
# ---------------------------------------------------------------------------


class TestRewriteFiles:
    def test_processes_all_files(self, tmp_path: Path) -> None:
        a = tmp_path / "a.md"
        b = tmp_path / "b.md"
        a.write_text("File A content.\n", encoding="utf-8")
        b.write_text("File B content.\n", encoding="utf-8")

        mock_client = _make_mock_client("Rewritten content.\n")

        results = rewrite_files([a, b], mock_client)  # pyright: ignore[reportArgumentType]

        assert len(results) == 2
        assert all(r.status == "rewritten" for r in results)

    def test_error_in_one_does_not_stop_others(self, tmp_path: Path) -> None:
        a = tmp_path / "a.md"
        b = tmp_path / "missing.md"  # does not exist
        c = tmp_path / "c.md"
        a.write_text("Content A.\n", encoding="utf-8")
        c.write_text("Content C.\n", encoding="utf-8")

        mock_client = _make_mock_client("Rewritten.\n")

        results = rewrite_files([a, b, c], mock_client)  # pyright: ignore[reportArgumentType]

        assert len(results) == 3
        assert results[0].status == "rewritten"
        assert results[1].status == "error"
        assert results[2].status == "rewritten"

    def test_serial_execution_order(self, tmp_path: Path) -> None:
        """Files must be processed in the order given (serial, not parallel)."""
        order: list[str] = []

        def ordered_chat(**kwargs: Any) -> ChatResult:  # pyright: ignore[reportAny]
            user: str = kwargs["user"]
            order.append(user[:6])  # first chars identify the file
            return _make_chat_result("Done.\n")

        mock_client = MagicMock()
        mock_client.chat.side_effect = ordered_chat
        mock_client.summary.api_calls = 0
        mock_client.summary.prompt_tokens = 0
        mock_client.summary.completion_tokens = 0
        mock_client.summary.cost_usd = 0.0

        files: list[Path] = []
        for i in range(3):
            p = tmp_path / f"file{i}.md"
            p.write_text(f"file{i} content here.\n", encoding="utf-8")
            files.append(p)

        rewrite_files(files, mock_client)  # pyright: ignore[reportArgumentType]

        # Verify processing happened in order (each call contained file identifier)
        assert len(order) == 3
        assert order[0].startswith("file0")
        assert order[1].startswith("file1")
        assert order[2].startswith("file2")


# ---------------------------------------------------------------------------
# Integration test (skipped unless MDD_AI_TEST=1)
# ---------------------------------------------------------------------------


@pytest.mark.integration
class TestRewriteIntegration:
    def test_live_rewrite(self, tmp_path: Path) -> None:
        from mdd.ai.client import Client

        src = tmp_path / "test.md"
        src.write_text(
            "# Test Document\n\nThis is a document that is very verbose and "
            "contains many unnecessary words that could be removed.\n",
            encoding="utf-8",
        )

        client = Client()
        result = rewrite_file(src, client)

        assert result.status == "rewritten"
        assert result.output_path is not None
        assert result.output_path.exists()


class TestProtectedRegionScaling:
    def test_unterminated_fence_protects_rest_of_document(self) -> None:
        text = "Prose\n\n```\nnever closed\nmore code\n"
        transformed, regions = extract_protected(text)
        assert len(regions) == 1
        assert regions[0].original == "```\nnever closed\nmore code\n"
        assert transformed == "Prose\n\n__MDD_PROTECTED_0__"

    def test_many_fence_openers_finish_quickly(self) -> None:
        import time

        text = "```x\n" * 40_000
        start = time.perf_counter()
        transformed, regions = extract_protected(text)
        elapsed = time.perf_counter() - start
        assert stitch_protected(transformed, regions) == text
        assert elapsed < 1.0, f"extract_protected took {elapsed:.3f}s"
